...
import { maskEmail, sendFollowUp } from "./comment.handler.js";
...
async function handleStartEmail(
  ...
  try {
    ...
    if (lead?.email) {
      ...
      await sendButtonDM(
        senderId,
        `Tengo tu email ${masked}. Te mando el link ahi?`,
        [
          {
            type: "postback",
            title: "Si, mandame ahi",
            payload: `confirm_email:${keywordId}`,
          },
          {
            type: "postback",
            title: "No, cambiar email",
            payload: `change_email:${keywordId}`,
          },
        ],
      );
...
async function handleConfirmEmail(
  ...
  try {
    ...
    logger.info(
      { senderId, keywordId, email: lead.email },
      "Email confirmed, followUp sent",
    );
  } catch (err) {
